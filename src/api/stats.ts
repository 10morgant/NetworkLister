...
export async function fetchStatsResources(): Promise<StatResources> {
    const res = await fetch(`${API_BASE}/stats/resources`);
    if (!res.ok) throw new Error('Network error');
    return res.json();
}

export async function fetchStatsOwner(): Promise<StatOwner> {
    const res = await fetch(`${API_BASE}/stats/owners`);
    if (!res.ok) throw new Error('Network error');
    return res.json();
}

export async function fetchStatsAge(): Promise<StatAge> {
    const res = await fetch(`${API_BASE}/stats/age`);
    if (!res.ok) throw new Error('Network error');
    return res.json();
}

export async function fetchStatsNetworks(): Promise<StatNetworks> {
    const res = await fetch(`${API_BASE}/stats/network`);
    if (!res.ok) throw new Error('Network error');
    return res.json();
}
