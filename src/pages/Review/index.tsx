...
import {
    Box, Group, Stack, Text, SegmentedControl, Table, Checkbox,
    Button, ActionIcon, Modal, Tooltip, Menu, Collapse, SimpleGrid,
    Badge,
} from '@mantine/core';
import { useDisclosure } from '@mantine/hooks';
import {
    IconArchive, IconTrash, IconAlertTriangle,
    IconDownload, IconFileTypeCsv, IconJson,
    IconChevronRight,
} from '@tabler/icons-react';
import { MACHINES as INITIAL_MACHINES, NETWORKS } from '@/data/mock';
...
export default function ReviewPage() {
    ...
    const handleConfirm = () => {
        if (confirmAction === 'delete') {
            ...
            setArchived(prev => { const n = new Set(prev); selected.forEach(ip => n.add(ip)); return n; });
            setMachines(prev => prev.filter(m => !selected.has(m.ip)));
        }
        setSelected(new Set());
        setExpanded(new Set());
    ...
    const triggerAction = (action: 'archive' | 'delete') => {
        setConfirmAction(action); openModal();
    };
            ...
            <Group px="md" py="xs" gap="sm"
                ...
                <Group gap="xs">
                    ...
                    <SegmentedControl size="xs" value={threshold}
                        onChange={v => { setThreshold(v); setSelected(new Set()); setExpanded(new Set()); }}
                                            ...
                                            <Group gap={4}>
                                                <Tooltip label="Archive" withArrow>
                                                    <ActionIcon size="xs" variant="light" color="yellow"
                                                        onClick={() => { setSelected(new Set([m.ip])); triggerAction('archive'); }}>
                                                        <IconArchive size={12} />
                                                    </ActionIcon>
                                                </Tooltip>
                                                <Tooltip label="Delete" withArrow>
                                                    <ActionIcon size="xs" variant="light" color="red"
                                                        onClick={() => { setSelected(new Set([m.ip])); triggerAction('delete'); }}>
                                                        <IconTrash size={12} />
                                                    </ActionIcon>
                                                </Tooltip>
                                            </Group>
