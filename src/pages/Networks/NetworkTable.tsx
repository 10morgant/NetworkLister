...
import {
    Box, Group, TextInput, SegmentedControl, Switch, Text,
    Table, Collapse, SimpleGrid, Stack,
    Center,
    Loader,
} from '@mantine/core';
...
import { fmtDate, fmtUptime } from './utils';
        ...
        <Stack gap={0} style={{ flex: 1, minHeight: 0, overflow: 'hidden' }}>
            ...
            <Group px="md" py="xs" gap="sm" style={{ borderBottom: '1px solid var(--mantine-color-dark-5)', flexShrink: 0 }} wrap="wrap">
                <TextInput placeholder="Search IP, name, owner…" leftSection={<IconSearch size={14} />}
                    size="xs" value={search} onChange={e => setSearch(e.currentTarget.value)} style={{ flex: '1 1 180px', maxWidth: 280 }} />
                <SegmentedControl size="xs"
                    data={['all', 'on', 'off', 'suspended']}
                    value={powerF} onChange={setPowerF} />
                {isCore && (
                    <Switch size="xs" label="Show empty IPs" checked={showEmpty} onChange={e => setShowEmpty(e.currentTarget.checked)} />
                )}
                            ...
                            return (
                                <>
                                    ...
                                    >
                                        ...
                                        <Table.Td fw={machine ? 500 : 400} c={machine ? undefined : 'dark.5'} ff="monospace">
